
------------------------##############---  COVID_19_India_Dashboard  ---###############-----------------------------------


Hey, 
    This is Deepak if you want to have a look on desktop click on the link below.

https://public.tableau.com/profile/deepakshende#!/vizhome/COVID-19INDIA_15871332806480/Covid-19INDIA?publish=yes


------------------------##############---  COVID_19_India_Dashboard  ---###############-----------------------------------

To make the dashboard on your system:

STEPS:

1. Download the COVID_19_India_Dashboard.zip from GitHub(If already downloaded skip this step).
2. Extract all the files of COVID_19_India_Dashboard.zip
3. Download Tableau Desktop from https://downloads.tableau.com/tssoftwareregistered/TableauDesktop-64bit-2020-2-0.exe
4. Install Tableau correctly and run it.
5. Open COVID-19 INDIA.twb in Tableau.
6. Add the dataset in the tableau and like refresh extract in tableau.
7. ENJOY 😁

Note:

1. Tableau desktop version used while creation of dashboard was "2020.2.0"


-----------------------##############---  COVID_19_India_Dashboard  ---###############-------------------------------------